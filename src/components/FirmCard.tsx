...
export default function FirmCard({ firm, dimmed = false }: FirmCardProps) {
  return (
    <div
      className={`h-full flex flex-col p-10 border bg-white transition-all duration-500 group ${
        dimmed ? 'opacity-30' : 'opacity-100 border-forest/10 hover:border-gold/30 hover:shadow-2xl hover:shadow-forest/5'
      }`}
    >
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-3">
          <div className="p-1.5 bg-forest/5 group-hover:bg-gold/10 transition-colors">
      ...
      <p className="text-forest/60 text-[15px] leading-[1.7] mb-6 font-light flex-grow">
        {firm.description}
      </p>
      ...
      <div className="pt-6 border-t border-forest/10 mt-auto">
        ...
        <div className="flex flex-wrap gap-2">
          {firm.coverage.map((region, idx) => (
            <span
              key={idx}
              className="px-2 py-1 bg-gold/10 text-dark-gold text-[11px] font-medium uppercase tracking-[0.1em] border border-gold/20"
            >
              {region}
            </span>
          ))}
